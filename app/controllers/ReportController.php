...
class ReportController extends \BaseController {
	...
	*/
    ...
    public function saleReport()
	{
		...
		$from=date('Y-m-d');
        $to=date('Y-m-d');
        return View::make('reports.details.saleReport',compact('from','to', 'subCompanies'));
	}
    public function viewSaleReport()
	{
	    ...
	    $to=$input['to'];
	    $com_id=$input['com_id'];
	    $reports= DB::select("select customerinfos.user_name as customer_name,customerinfos.full_name as customer_full_name,empinfos.user_name as invoiced_employee, each_item.*,(sum(each_item.item_profit) - (each_item.discount+each_item.point_use_taka)) as profit  
				from
				...
				where saleinvoices.com_id = $com_id
...
//
		...
		return View::make('reports.summary.fullReport', compact('title','from','to','other_expense','other_income','damage','purchasereturn','purchase','salereturn','sale', 'totalGodwonTk', 'totalStockTk', 'cusDueAmount', 'supDueAmount', 'cusDuePayment', 'supDuePayment'));
	}
