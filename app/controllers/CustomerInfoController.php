...
class CustomerInfoController extends \BaseController {
    ...
    public function index() {
            ...
            // $count = 0;
            // DB::beginTransaction();
            // foreach($cusDue as $key => $due){
            //     // echo ;
            //     $update=DB::table('customerinfos')
            //         ->where('cus_id', $due['Customer_ID'])
            //         ->update([
            //             'due' => $due['Due'],
...
//          $data = Input::all();
    ...
    public function store() {
        ...
        try {
            ...
            $insert = DB::select("insert into customerinfos (cus_card_id,cus_type_id,branch_id,full_name,user_name,password,mobile,email,national_id,permanent_address,present_address,created_at,created_by) values
                      ( ifnull(1+(
                        SELECT right(sale_inv.cus_card_id, 7) AS LAST7 FROM customerinfos as sale_inv
                        order by LAST7 desc limit 1),'1000000'),'$cus_type_id','$branch_id','$full_name','$user_name','$password','$mobile','$email','$national_id','$permanent_address','$present_address','$created_at','$created_by')");
