...
class SaleController extends \BaseController
{
    ...
    {
        // return Hash::make('metro!!');
        Session::forget('receipt_item_infos');
        Session::forget('receipt_info');
        Session::forget('company_info');
        // Session::forget('saleItems');

        $sub_companies = DB::table('sub_companies')
            ->select('company_name','id')
            ->where('status',1)
            ->get();

        foreach ($sub_companies as $key => $value) {
            $subCompanies[$value->id] = $value->company_name;
        }
        ...
        return View::make('sale.saleForm',compact('items','customers','payment_type','subCompanies'));
    }
    public function autoItemSuggest()
    {
        $term = Input::get('q');
        $cus = Session::get('sale_invoice_info.cus_id');
        $search_items = DB::table('stockitems')
            ...
            ->join('customerarticles', 'iteminfos.item_id', '=', 'customerarticles.item_id')
            ->where('stockitems.status', '=', 1)
            ->where('priceinfos.status', '=', 1)
            ->where('iteminfos.item_name', 'LIKE', '%'. $term .'%')
            ->orWhere('iteminfos.upc_code', '=', $term)
            ->where('customerarticles.customer_id', '=', $cus)
            ->orWhere('customerarticles.article_name', 'LIKE', '%'. $term .'%')
            ->groupBy('stockitems.item_id')
            ->get();
        foreach($search_items as $key => $item):
            $upc_code  =$item->upc_code;
            $item_name = $item->item_name." (".$upc_code.")"."(".$item->article_name.")";
    ...
    public function saleItemAddChart()
    {
        ...
        // $search_key = Input::get('item_id');\
        // $searchItem = DB::table('iteminfos')
        //     ->where('');

        // return response()->json(['success' => true, 'item' => $item_id]);
        $datas = DB::table('stockitems as s')
            ->select('s.stock_item_id', 's.item_id', 's.price_id', 'i.item_name','i.tax_amount','i.offer','i.unit', 'p.purchase_price','p.sale_price','ca.customer_article_id','ca.article_name', DB::raw('SUM(s.available_quantity) as available_quantity, count(*) as row_no'))
            ->leftjoin('iteminfos as i', 's.item_id', '=', 'i.item_id')
            ->leftJoin('priceinfos as p', 'p.price_id', '=', 's.price_id')
            ->leftJoin('customerarticles as ca', 'i.item_id', '=', 'ca.item_id')
            ->where('s.status', '=', 1)
            ->where('s.branch_id', '=', Session::get('branch_id'))
            ->where('ca.customer_id', '=', Session::get('sale_invoice_info.cus_id'))
            // ->where('ca.item_id', '=', Session::get('sale_invoice_info.cus_id'))
        ...
        foreach($datas as $data){
            ...
            if(array_key_exists('customer_article_id', $data)){
                $item_info['customer_article_id']  = $data->customer_article_id;
                $item_info['article_name']  = $data->article_name;
            }else{
                $item_info['customer_article_id']  = '';
                $item_info['article_name']  = '';
            }
  ...
  public function autoCustomerSuggest(){
    ...
    {   
        ...
        if(!Session::get("saleItems"))
            ...
            try {
                ...
                $com_id = $vdata['com_id'];
                $po_no=$vdata['po_no'];
                $supply_code=$vdata['supply_code'];
                $customer_name = '';
                if (isset($vdata['customer_name'])) {
                    $customer_name = $vdata['customer_name'];
                }
                $branch_name=$vdata['branch_name'];
                ...
                $insert = DB::select("insert into saleinvoices (branch_id,sale_invoice_id,cus_id,payment_type_id,po_no,com_id,branch_name,customer_name,supply_code,discount,amount,pay,due,pay_note,date,created_by,created_at) values
               ('$branch_id',ifnull(concat('$date',1+(
                SELECT right(sale_inv.sale_invoice_id, 8) AS LAST8 FROM saleinvoices as sale_inv
                  ...
                  '$cus_id','$payment_type_id','$po_no','$com_id','$branch_name','$customer_name','$supply_code','$discount','$amount','$pay','$due','$pay_note','$transaction_date','$created_by','$created_at')");
        ...
        ### --- Item Sale Part --- ###
            ...
            $cus_name = Session::get("sale_invoice_info.full_name");
            $cus_name = explode('#',$cus_name);
            if ($customer_name != '') {
                $receipt_info['customer_full_name']=$customer_name;
            }else{
                $receipt_info['customer_full_name']= $cus_name[0];
            }
            ...
            $receipt_info['po_no']=$po_no;
            $receipt_info['supply_code']=$supply_code;
            $receipt_info['branch_name']=$branch_name;
            ...
            $sub_company_info=DB::table('sub_companies')
                ->where('id', $vdata['com_id'])
                ->get();
            
            DB::commit();
            Session::put('receipt_item_infos',$receipt_item_infos);
            Session::put('receipt_info',$receipt_info);
            Session::put('company_info',$sub_company_info);
            /*print "<pre>";
            print_r(Session::get("receipt_info"));
            return;*/
            return Redirect::to('sale/receipt');
        } catch (Exception $e) {
            return $e;
            DB::rollback();
    ...
    {      
        // echo "<pre>"; print_r(Session::get('receipt_item_infos')); exit();
        $com = Session::get('company_info')[0]->id;
        if ($com == 1) {
            return View::make('sale.invoiceMrTraders');
        }elseif ($com == 2) {
            return View::make('sale.invoiceMbsCorp');
        }elseif ($com == 3) {
            return View::make('sale.invoiceFamousCorp');
        }
            return View::make('sale.saleReceipt');
    }
    public function salesChalan()
    {   
        return View::make('sale.salesChalan');
    }
  ...
  // ============== @@ Sales Order @@ =================
    ...
    public function salesOrder()
    {
        ...
        return View::make('sale.saleOrderForm',compact('items','customers','payment_type','subCompanies'));
    }
    ...
    public function itemAddTochartForSaleOrder()
    {
        $item_id = Input::get('item_id');
        $datas = DB::table('stockitems as s')
            ->select('s.stock_item_id', 's.item_id', 's.price_id', 'i.item_name','i.tax_amount','i.offer','i.unit','i.carton', 'p.purchase_price','p.sale_price','ca.customer_article_id','ca.article_name', DB::raw('SUM(s.available_quantity) as available_quantity, count(*) as row_no'))
            ->leftjoin('iteminfos as i', 's.item_id', '=', 'i.item_id')
            ->leftJoin('priceinfos as p', 'p.price_id', '=', 's.price_id')
            ->leftJoin('customerarticles as ca', 'i.item_id', '=', 'ca.item_id')
            ->where('s.status', '=', 1)
    ...
    $key=$vdata['key'];
        ...
        $data = DB::table('stockitems as s')
            ->select('s.stock_item_id','ca.customer_article_id','ca.article_name', 's.item_id', 's.price_id', 'i.item_name','i.tax_amount','i.offer','i.unit','i.carton', 'p.purchase_price','p.sale_price', DB::raw('SUM(s.available_quantity) as available_quantity, count(*) as row_no'))
            ->leftjoin('iteminfos as i', 's.item_id', '=', 'i.item_id')
            ->leftJoin('priceinfos as p', 'p.price_id', '=', 's.price_id')
            ->leftJoin('customerarticles as ca', 'i.item_id', '=', 'ca.item_id')
            ->where('s.status', '=', 1)
    ...
    {   
        ...
        if(!Session::get("saleOrderItems"))
            ...
            try {
                ...
                $point = (int)($payable_amount*0.01);
                $date = date('ymd');
                $insert = DB::select("insert into saleinvoices_order (branch_id,sale_order_invoice_id,cus_id,com_id,payment_type_id,discount,amount,pay,po_no,branch_name,supply_code,due,pay_note,date,created_by,created_at) values
               ('$branch_id',ifnull(concat('$date',1+(
                SELECT right(sale_inv.sale_order_invoice_id, 8) AS LAST8 FROM saleinvoices_order as sale_inv
                  ...
                  order by LAST8 desc limit 1)),concat('$date','10000000')),
                  '$cus_id','$com_id','$payment_type_id','$discount','$amount','$pay','$po_no','$branch_name','$supply_code','$due','$pay_note','$transaction_date','$created_by','$created_at')");
             $last_insert_id = DB::getPdo()->lastInsertId();
        ...
        // echo "<pre>"; print_r(Session::get("saleOrderItems")); exit();
            ...
            // Session::put('company_info',$sub_company_info);
            DB::commit();
            return Redirect::to('sale/saleOrderReceipt')->with('receipt_item_infos', $receipt_item_infos)->with('receipt_info', $receipt_info)->with('company_info', $sub_company_info);
        } catch (Exception $e) {
            // return $e;
            DB::rollback();
    ...
    public function sendOrderToSale($saleReportInvoiceId){
        ...
        $item_infos = DB::table('itemsales_order')
            ...
            ->leftJoin('customerarticles as ca', 'ii.item_id', '=', 'ca.item_id')
            ->select(['itemsales_order.quantity', 'itemsales_order.amount', 'ii.item_name','ii.upc_code','ii.item_id', 'pi.sale_price','pi.price_id','customerinfos.national_id','saleinvoices_order.cus_id','saleinvoices_order.com_id','saleinvoices_order.po_no','saleinvoices_order.branch_name','saleinvoices_order.supply_code','ca.customer_article_id','ca.article_name'])
            ->where('itemsales_order.sale_order_invoice_id', $saleReportInvoiceId)
            ->where('itemsales_order.status', 1)->groupBy('ii.item_id')
            ->get(); 
        ...
        foreach($item_infos as $key => $order_item_info){
            ...
            if($qdata){
                ...
                Session::put("sale_order_invoice_info.com_id", $order_item_info->com_id);
                Session::put("sale_order_invoice_info.po_no", $order_item_info->po_no);
                Session::put("sale_order_invoice_info.branch_name", $order_item_info->branch_name);
                Session::put("sale_order_invoice_info.supply_code", $order_item_info->supply_code);
            }
            ...
            if(count($datas)>0){
                foreach($datas as $data){
                    ...
                    $item_info['article_name'] = $order_item_info->article_name;
...
// ,'ca.customer_article_id','ca.article_name'
        $item_infos = DB::table('itemsales_order')
            ->join('saleinvoices_order', 'itemsales_order.sale_order_invoice_id', '=', 'saleinvoices_order.sale_order_invoice_id')
            ->leftJoin('iteminfos as ii', 'ii.item_id', '=', 'itemsales_order.item_id')
            ->leftJoin('priceinfos as pi', 'pi.price_id', '=', 'itemsales_order.price_id')
