...
<?	$created = substr($receipt_info->created_at , 0, 10); ?>
	<!--Receipt-->	  	
		 ...
		 @if ($receipt_info)
			...
			<table>
				<tbody>
					...
					@if(!empty($receipt_info->supplier_name))
					<tr>
					...
					<tr>
						...
						<td>&nbsp;&nbsp; : &nbsp;</td>
						<td align="left">{{ $receipt_info->sup_r_invoice_id }}</td>
					</tr>
					<tr>
		...
		<table class="table-sales-receipt">
			...
			<tbody>
				<? $i = 0; ?>
				@foreach($receipt_return_item_infos as $receipt_return_item_info)
				<? $i++; ?>
				<tr>
					<td>{{ $i }}</td>
					...
					<td>{{ $receipt_return_item_info->amount }}</td>
				</tr>
				@endforeach
			</tbody>
		</table>
		...
		<article style="clear:both; text-align: center;">
			...
			<div align="center">
				{{ DNS1D::getBarcodeHTML($receipt_info->sup_r_invoice_id, "C128", 1, 25) }}					
				<strong>{{ $receipt_info->sup_r_invoice_id }}</strong>				
			</div>
			<p style="float:right;"><strong>Developed By : UB Automation</strong></p>
